...
def welcomemsg_chat():
    ...
    db_handler.insert_many([{"request_user_id": request_user_id}, {"conversation": conversation_list_history},
                       {"time": now_india.strftime(fmt)}])
    db_handler.update_many({"request_user_id": request_user_id}, {
...
def mobilenumber_asking():
    if ('msg' in request.args):
        ...
        post_data_lead_create.update_many({"phone": mobilenumber_asking_msg})
...
def loan_ammount_asking_coversation():
    if ('msg' in request.args):
        ...
        post_data_lead_create.update_many({"loanAmount": loan})
...
if __name__ == "__main__":
    app.run(host='localhost', port=5002, threaded=True, debug=True)
